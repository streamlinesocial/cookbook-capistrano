...
unless node["apache"].nil?
    ...
    unless node["apache"]["user"] == deploy_user
        ...
        execute "capistrano-directory-setfacl-set-initial" do
            action :nothing
            command "setfacl -R -m u:#{node["apache"]["user"]}:rwX -m u:#{deploy_user}:rwX #{node['capistrano']['deploy_to_root']}"
            subscribes :run, "directory[#{node['capistrano']['deploy_to_root']}]", :immediately
        end

        execute "capistrano-directory-setfacl-set-default" do
            action :nothing
            command "setfacl -dR -m u:#{node["apache"]["user"]}:rwX -m u:#{deploy_user}:rwX #{node['capistrano']['deploy_to_root']}"
            subscribes :run, "directory[#{node['capistrano']['deploy_to_root']}]", :immediately
        end
    end
end
